...
CREATE TABLE Hoteles (
    ...
    descripcion VARCHAR(600),
    url_pagina VARCHAR(2083),  -- VARCHAR(MAX) no existe en MySQL, se usa 2083 como límite máximo de URL
    url_img VARCHAR(2083),     -- VARCHAR(MAX) no existe en MySQL, se usa 2083 como límite máximo de URL
    dato1 VARCHAR(50),
    dato2 VARCHAR(50),
    id_destino INT,
...
CREATE PROCEDURE InsertarUsuario (
    ...
    IN contrasena VARCHAR(225),
    IN fecha_registro DATE
)
BEGIN
    INSERT INTO Usuarios (nombre, apellido, correo, contrasena, fecha_registro)
    VALUES (nombre, apellido, correo, Usuarios.contrasena, fecha_registro);
END //
...
BEGIN
    ...
    VALUES (NEW.id_usuario, 'Actualización', NOW());
END //

CREATE TRIGGER ActualizarInformacionSitiosTuristicos
AFTER UPDATE ON Atractivos_Turisticos
FOR EACH ROW
BEGIN
    INSERT INTO Historial_Sitios_Turisticos (id_atractivo_turistico, accion, fecha)
    VALUES (NEW.id_atractivo_turistico, 'Actualización', NOW());
END //
...
BEGIN
    ...
    VALUES (OLD.id_usuario, 'Eliminación', NOW());
END //
...
BEGIN
    ...
    VALUES (NEW.id_destino, 'Actualización', NOW());
END //
...
BEGIN
    INSERT INTO Historial_Lugares_Ofertados (id_destino, accion, fecha)
    VALUES (NEW.id_destino, 'Nuevo Registro', NOW());
END //
